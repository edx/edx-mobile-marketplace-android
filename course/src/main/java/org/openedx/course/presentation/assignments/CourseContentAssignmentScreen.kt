...
import org.openedx.course.presentation.CourseMocks
...
private fun AssignmentGroupSection(
    ...
    val progress = Progress(
        ...
        assignmentsCompleted = assignments.filter { it.isCompleted() }.size
    )
    val description = stringResource(
        id = R.string.course_completed_of,
        progress.assignmentsCompleted,
...
private fun AssignmentButton(assignment: Block, isSelected: Boolean, onClick: () -> Unit) {
    ...
    val icon = when {
        assignment.isCompleted() -> painterResource(id = R.drawable.ic_core_check)
        isDuePast -> painterResource(id = R.drawable.ic_core_watch_later)
        else -> null
    }
    ...
    Column(
        ...
        if (isSelected) {
            Icon(
                ...
                painter = painterResource(id = R.drawable.ic_core_pointer),
