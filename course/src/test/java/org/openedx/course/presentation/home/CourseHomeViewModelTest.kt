...
class CourseHomeViewModelTest {
    ...
    fun `logVideoClick analytics event`() = runTest {
        ...
        verify {
            analytics.logEvent(
                ...
                match {
                    it[CourseAnalyticsKey.NAME.key] ==
    ...
    fun `CourseProgressLoaded notifier event`() = runTest {
        ...
        coEvery { interactor.getCourseStatusFlow(courseId) } returns flow {
            emit(CoreMocks.mockCourseComponentStatus)
        }

        coEvery {
            interactor.getCourseDatesFlow(courseId)
        } returns flow {
            emit(CoreMocks.mockCourseDatesResult)
        }
        ...
        coEvery {
            ...
            emit(CoreMocks.mockCourseProgress)
        }

        every { courseNotifier.notifier } returns flow {
            emit(CourseProgressLoaded)
        }
